...
const CREATE_SYSTEM =
  ...
  "TASTE — warm paper product surface, not a generic SaaS landing: background #f4efe6, ink #1c1915, muted #4a433a, line #ddd4c6, terracotta #c45c38, cream #fff7f0. Serif body (Palatino/Georgia), system-ui for controls. Keep CSS minimal (inputs/fields/layout helpers only); cozy-* elements supply the chrome. " +
  "CHROME — exactly one top header via <cozy-app kicker heading lede>: kicker = app-kind label (Kanban, Chat, Habits, Calendar, Notes, or Cozy Studio), heading = short title distilled from the brief, lede = one-sentence summary from the brief. NEVER add a second <header>, hero band, nav bar, gradient banner, or duplicate h1 outside cozy-app. No purple/blue gradient heroes, no stock imagery, no lorem ipsum — every visible string must come from the brief in the brief's language (Slovak brief → Slovak UI, placeholders, column names, and sample cards; set <html lang> accordingly). " +
  "KIND — infer one interactive surface: kanban → add-card form + <cozy-board> with cozy-column name=Inbox/Doing/Done and cozy-card starters relevant to the brief; chat → cozy-msg log + send form; habits → weekly grid inside cozy-card rows; calendar → cozy-board with month grid + notes column; notes → cozy-board library + editor columns; landing → cozy-app + one cozy-btn CTA only (no fake feature grid or pricing table). Kanban columns default to Inbox, Doing, Done unless the brief specifies others in the same language. Seed 2–3 starter cards with brief-specific titles, not generic English filler. " +
  "Vanilla JS only. Wrap localStorage in try/catch. No Tailwind, no CDNs, no external scripts, no Node APIs, no Vite." +
  WC;
